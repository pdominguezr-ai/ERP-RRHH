    ...
    },
    {
      ...
      render: (row) => {
        ...
        if (row.operacion === 'UPDATE' && row.datos_antes && row.datos_despues) {
          ...
          const keys = ['codigo', 'nombres', 'apellidos', 'estado', 'tipo', 'total_neto', 'titulo'].filter(
            (k) => k in desp
          );
        ...
        return (
          <div className="max-w-xs sm:max-w-sm overflow-hidden text-xs text-gray-600 font-mono space-y-1 py-1">
            ...
            {changes.length > 3 && (
              <div className="text-gray-400 italic text-[10px]">+ {changes.length - 3} campos más...</div>
            )}
          </div>
        );
      },
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <DataTable
        data={logs}
        columns={columns}
        searchKeys={['tabla', 'operacion', 'registro_id']}
