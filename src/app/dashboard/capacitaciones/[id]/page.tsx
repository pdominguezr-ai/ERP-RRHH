...
export default async function DetalleCapacitacionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const columns: Column<CapacitacionEmpleado>[] = [
    ...
    },
    {
      ...
      header: 'Estado',
      render: (row) => (
        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${row.completada ? 'bg-green-50 text-green-700 ring-1 ring-inset ring-green-600/20' : 'bg-yellow-50 text-yellow-700 ring-1 ring-inset ring-yellow-600/20'}`}>
